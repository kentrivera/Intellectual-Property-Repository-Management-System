// Global state
// Use 0 as a virtual root folder (avoids colliding with real DB folder ids)
...
async function fetchJson(url, options = {}) {
    ...
    try {
        ...
        // Common cause: auth redirect HTML, PHP warnings, etc.
...
async function loadContent(folderId, addToHistory = true) {
    ...
    try {
        ...
        showNotification(error.message || 'Error loading content', 'error');
    } finally {
        hideLoading();
    }
}
...
async function loadBreadcrumb(folderId) {
    try {
        const breadcrumb = await fetchJson(`api_folders.php?action=get_breadcrumb&folder_id=${folderId}`);
        ...
        showNotification(error.message || 'Error loading breadcrumb', 'error');
    }
}
...
async function loadFolders(parentId) {
    try {
        const folders = await fetchJson(`api_folders.php?action=get_folders&parent_id=${parentId}`);
        ...
        showNotification(error.message || 'Error loading folders', 'error');
        const el = document.getElementById('foldersList');
        if (el) el.innerHTML = '<p class="text-gray-500 text-sm italic">Unable to load folders</p>';
    }
}
...
async function loadFiles(folderId) {
    try {
        const [files, folders] = await Promise.all([
            fetchJson(`api_folders.php?action=get_files&folder_id=${folderId}`),
            fetchJson(`api_folders.php?action=get_folders&parent_id=${folderId}`)
        ]);
        ...
        showNotification(error.message || 'Error loading files', 'error');
        const el = document.getElementById('filesList');
        if (el) el.innerHTML = '<p class="text-gray-500 text-sm italic">Unable to load items</p>';
    }
}
...
async function createFolder(event) {
    ...
    try {
        await fetchJson('api_folders.php?action=create_folder', {
            method: 'POST',
            headers: {
...
async function updateFolder(event) {
    ...
    try {
        await fetchJson('api_folders.php?action=update_folder', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        await Swal.fire({
            ...
            text: error.message || 'Unable to rename folder.',
            confirmButtonColor: '#16a34a'
        });
    } finally {
        hideLoading();
...
async function deleteFolder(folderId) {
    ...
    try {
        await fetchJson(`api_folders.php?action=delete_folder&id=${folderId}`);
        ...
        await Swal.fire({
            icon: 'error',
            title: 'Error',
            text: error.message || 'Error deleting folder.',
            confirmButtonColor: '#16a34a'
        });
    }
}
...
async function uploadFile(event) {
    ...
    try {
        const result = await fetchJson('upload.php', {
            method: 'POST',
            body: formData
        });
...
async function updateFile(event) {
    ...
    try {
        await fetchJson('api_files.php?action=update_file', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function deleteFile(fileId) {
    ...
    try {
        await fetchJson(`api_files.php?action=delete_file&id=${fileId}`);
...
async function loadAllFoldersTree() {
    try {
        ...
        if (folders.error) {
            ...
            const rootResponse = await fetch('api_folders.php?action=get_folders&parent_id=0');
...
function buildTree(folders, parentId) {
    return folders
        .filter(f => {
            if (parentId === null) {
                return f.parent_id === null || f.parent_id == 0 || f.id == 0;
            }
